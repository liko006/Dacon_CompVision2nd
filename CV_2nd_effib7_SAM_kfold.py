...
def train(fold: int, verbose: int = 30):
    ...
    valid_loader = DataLoader(validset, batch_size=16, shuffle=False, num_workers=2)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > num_epochs-20:
            torch.save(model.state_dict(), f'models/effinet_b7_SAM_CosLR-f{fold}-{epoch}.pth')
...
def load_model(fold: int, epoch: int, device: torch.device = 'cuda'):
    ...
    model = EfficientNetModel().to(device)
    model.load_state_dict(torch.load(f'models/effinet_b7_SAM_CosLR-f{fold}-{epoch}.pth'))
...
def test(device: torch.device = 'cuda'):
    ...
    model1 = load_model(0, 29)
    model2 = load_model(1, 29)
    model3 = load_model(2, 29)
    model4 = load_model(3, 29)
    model5 = load_model(4, 29)
    ...
    submit.to_csv('effinet_b7_SAM_CosLR-kfold.csv', index=False)

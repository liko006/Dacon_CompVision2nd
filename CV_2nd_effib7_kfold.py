...
def train(fold: int, verbose: int = 30):
    ...
    train_loader = DataLoader(trainset, batch_size=24, shuffle=True, num_workers=0)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > num_epochs-20:
            torch.save(model.state_dict(), f'models/effinet_b7-f{fold}-{epoch}.pth')
...
def load_model(fold: int, epoch: int, device: torch.device = 'cuda'):
    ...
    model = EfficientNetModel().to(device)
    model.load_state_dict(torch.load(f'models/effinet_b7-f{fold}-{epoch}.pth'))
...
def test(device: torch.device = 'cuda'):
    ...
    submit.to_csv('effinet_b7-kfold.csv', index=False)

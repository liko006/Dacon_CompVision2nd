...
def train(fold: int, verbose: int = 30):
    ...
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2, eta_min=0.001, last_epoch=-1)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > num_epochs-20:
            torch.save(model.state_dict(), f'models/effinet_b4_SAM_CosLR-f{fold}-{epoch}.pth')
...
def load_model(fold: int, epoch: int, device: torch.device = 'cuda'):
    ...
    model = EfficientNetModel().to(device)
    model.load_state_dict(torch.load(f'models/effinet_b4_SAM_CosLR-f{fold}-{epoch}.pth'))

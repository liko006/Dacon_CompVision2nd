...
def train(fold: int, verbose: int = 30) -> None:
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > num_epochs-20 and epoch < num_epochs-1:
            torch.save(model.state_dict(), f'resnet50-f{fold}-{epoch}.pth')
...
def load_model(fold: int, epoch: int, device: torch.device = 'cuda') -> nn.Module:
    ...
    model = MnistModel().to(device)
    model.load_state_dict(torch.load(f'resnet50-f{fold}-{epoch}.pth'))
...
def test(device: torch.device = 'cuda'):
    ...
    model1 = load_model(0, 40)
    model2 = load_model(1, 40)
    model3 = load_model(2, 40)
    model4 = load_model(3, 40)
    model5 = load_model(4, 40)
    ...
    submit.to_csv('resnet50-e40-kfold.csv', index=False)
